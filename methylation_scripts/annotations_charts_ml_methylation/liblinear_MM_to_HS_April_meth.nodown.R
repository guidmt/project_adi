...
for(lrmt in 1:length(list_res_ml_to_use)){
	...
	pdf(paste(current_ml_res,"PCA_mm_to_hs_prob_meth.NODOWNSAMPLING.pdf",sep="_"),width=16)
...
pdf("LGG_boxplot_mm_to_hs_prob_meth.NODOWNSAMPLING.pdf")
...
pdf("GBM_LGG_boxplot_mm_to_hs_prob_meth.NODOWNSAMPLING.pdf")
...
for(cira in 1:10000){
	...
	lgg_ml_res_cira<-libLinear_mouse_hsApril(input_ml5_new,input_mm2,list_ann=list_ann,type_tumours="LGG",prefix_output="LGG",suffix_output="DMRs_hyper_hypo",dwn_sampling=F)
	acc_lgg<-t(data.frame(lgg_ml_res_cira[[4]][,1]))
	colnames(acc_lgg)<-rownames(lgg_ml_res_cira[[4]])
	lgg_acc_ci<-rbind(lgg_acc_ci,acc_lgg)

	gbm_lgg_ml_res_cira<-libLinear_mouse_hsApril(input_ml5_new,input_mm2,list_ann=list_ann,type_tumours=c("GBM","LGG"),prefix_output="GBM_LGG",suffix_output="DMRs_hyper_hypo",dwn_sampling=F)
...
write.table(CI_all,"mm_to_hs_ConfidenceIntervalsAccuracyLiblinear_NoDownsampling.Aug2021.txt",quote=F,sep="\t")
