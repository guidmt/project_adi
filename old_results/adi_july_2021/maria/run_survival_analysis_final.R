...
for(i in 1:nrow(sampleinfo)){
  ...
  group_of_start<-names(matrices_for_gsva)[1]
  ...
  for(an_gs in group_of_start){
    ...
    temp_ann_coxph<-get(ann_of_start[an_gs])
  ...
  for(an_gs2 in 1:length(matrices_for_gsva)){
      ...
      for(thr_cox in list_thr_cox){
        ...
        resCox_genes<-data.frame(resCox[resCox$LogRank<thr_cox,"Variable"])[,1]
        resCox_genes_select<-resCox[which(resCox$LogRank<thr_cox),]
        ...
        for(type_degs_cox in list_degs_cox){
          ...
          res_os<-plotSurvival2(TCGA_surv=input_os_from_cox,score_to_use="global_score",suffix_output=paste("gsva_aut.thr.",type_degs_cox))
